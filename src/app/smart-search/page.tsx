...
import { addBookmark, getBookmarks, removeBookmark, updateBookmark, Bookmark as BookmarkType } from '@/lib/bookmark-storage';
import { ArrowLeft, Search, Star, History, X, ExternalLink, Filter, Mic, Camera, Command, BookOpen, Volume2, Bookmark } from 'lucide-react';
...
export default function SmartSearchPage() {
    ...
    const [userBookmarks, setUserBookmarks] = useState<Map<string, BookmarkType>>(new Map());

    // Compatibility for existing check
    const bookmarks = useMemo(() => new Set(Array.from(userBookmarks.keys())), [userBookmarks]);
    ...
    useEffect(() => {
        ...
        const loadBookmarks = () => {
            ...
            const bmMap = new Map<string, BookmarkType>();
            bmList.forEach(b => bmMap.set(b.wordId, b));
            setUserBookmarks(bmMap);
        };
        loadBookmarks();
    ...
    const results = useMemo(() => {
        ...
        let filtered = allWords.filter(word => {
            ...
            if (showOnlyBookmarked) {
                if (!bookmarks.has(`${word.sourcePhase}-${word.id}`)) return false;
            }
    ...
    const handleToggleSave = (word: any) => {
        ...
        if (userBookmarks.has(id)) {
            removeBookmark(id);
            const newMap = new Map(userBookmarks);
            newMap.delete(id);
            setUserBookmarks(newMap);
        } else {
            addBookmark({ ...word, id }, 'default');
            // Refresh to get full object if needed, or mock it
            const newBm = getBookmarks().find(b => b.wordId === id);
            if (newBm) {
                const newMap = new Map(userBookmarks);
                newMap.set(id, newBm);
                setUserBookmarks(newMap);
    ...
    const handleToggleStar = (word: any) => {
        const id = `${word.sourcePhase}-${word.id}`;
        const bm = userBookmarks.get(id);

        if (bm) {
            // Toggle starred tag
            const isStarred = bm.tags.includes('starred');
            const newTags = isStarred
                ? bm.tags.filter(t => t !== 'starred')
                : [...bm.tags, 'starred'];

            updateBookmark(id, { tags: newTags });

            const newMap = new Map(userBookmarks);
            newMap.set(id, { ...bm, tags: newTags });
            setUserBookmarks(newMap);
        } else {
            // Add and Star
            addBookmark({ ...word, id }, 'default');
            updateBookmark(id, { tags: ['starred'] });
        ...
        <main className="min-h-screen bg-[#0A0A0F] text-slate-200 font-sans flex flex-col">
            ...
            <div className="sticky top-0 z-30 bg-[#0A0A0F]/95 backdrop-blur-md border-b border-slate-800 px-4 py-4 md:px-6">
                ...
                <div className="max-w-5xl mx-auto flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                    <Button
                        variant={activeFilter === 'all' && !showOnlyBookmarked ? 'default' : 'outline'}
                        onClick={() => { setActiveFilter('all'); setShowOnlyBookmarked(false); }}
                        className={`rounded-full h-8 text-xs ${activeFilter === 'all' && !showOnlyBookmarked ? 'bg-violet-600 hover:bg-violet-500' : 'border-slate-700 text-slate-400'}`}
                    >
                        All Types
                    </Button>
                    <Button
                        ...
                        className={`rounded-full h-8 text-xs gap-1 ${showOnlyBookmarked ? 'bg-yellow-500/20 text-yellow-500 hover:bg-yellow-500/30 border-yellow-500/50' : 'border-slate-700 text-slate-400'}`}
                    >
                        <Star className={`w-3 h-3 ${showOnlyBookmarked ? 'fill-current' : ''}`} />
                        Bookmarked
                    </Button>
            ...
            <div className="flex-1 max-w-5xl mx-auto w-full p-4 md:p-6">
                ...
                {searchTerm && (
                    <div className="space-y-4">
                        ...
                        {results.map((word) => (
                            <div key={`${word.sourcePhase}-${word.id}`} className="group bg-slate-900/40 border border-slate-800 rounded-2xl p-6 hover:border-violet-500/30 hover:bg-slate-900/60 transition-all">
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    <div className="flex flex-wrap items-center gap-3">
                                        ...
                                        <span className="text-sm font-serif italic text-slate-500">{word.partOfSpeech}</span>
                                        <Badge variant="outline" className="border-slate-700 text-slate-500 text-[10px] h-5 px-2">
                                            Phase {word.sourcePhase}
                                        </Badge>

                                        {/* Speaker Button - Next to Title */}
                                        <button
                                            onClick={(e) => handleSpeak(word.term, e)}
                                            className="p-1.5 rounded-full bg-slate-800/50 text-slate-400 hover:text-violet-400 hover:bg-violet-500/10 transition-all"
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        <button
                                            ...
                                            ${userBookmarks.get(`${word.sourcePhase}-${word.id}`)?.tags.includes('starred')
                                                    ? 'text-yellow-400 bg-yellow-400/10'
                                                    : 'text-slate-600 hover:text-yellow-400 hover:bg-slate-800'}
                                        `}
                                            title="Add to Favorites"
                                        >
                                            <Star className={`w-5 h-5 ${userBookmarks.get(`${word.sourcePhase}-${word.id}`)?.tags.includes('starred') ? 'fill-current' : ''}`} />
                                        </button>
                                        ...
                                        <button
                                            ...
                                            className={`p-2 rounded-lg transition-all 
                                            ${userBookmarks.has(`${word.sourcePhase}-${word.id}`)
                                                    ? 'text-cyan-400 bg-cyan-400/10'
                                                    : 'text-slate-600 hover:text-cyan-400 hover:bg-slate-800'}
                                        `}
                                            title="Bookmark"
                                        >
                                            <Bookmark className={`w-5 h-5 ${userBookmarks.has(`${word.sourcePhase}-${word.id}`) ? 'fill-current' : ''}`} />
                                        </button>
                                    </div>
                                </div>
